...
Use the provided development environment file or create a custom one:

```bash
ln -s .env.development .env.local
```

Or edit `.env.development` with your Clerk keys:
...
Use the provided backend development environment:
...
Or edit `backend/.env.development` with your Clerk Secret Key:

```env
CLERK_SECRET_KEY=sk_test_...
# Other vars like DATABASE_URL are already configured
```
...
Once these are set up, you can run the application locally (`bun run dev` in the root, and `uvicorn app.main:app --port 8080` in `backend/`) and navigate to `/admin`!
