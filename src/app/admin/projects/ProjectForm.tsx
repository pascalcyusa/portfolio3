...
export default function ProjectForm({ initialData }: { initialData?: any }) {
  const [formData, setFormData] = useState({
    ...
    design_process: initialData?.design_process || "",
    overview: initialData?.overview || [],
    technical_details: initialData?.technical_details || [],
    challenges: initialData?.challenges || [],
    outcomes: initialData?.outcomes || [],
    personal_contribution: initialData?.personal_contribution || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If editing, we first delete the old one because the backend doesn't have a PUT endpoint
      if (initialData?.id) {
         await fetch(`${apiUrl}/projects/${initialData.id}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    <form onSubmit={handleSubmit} className="max-w-4xl space-y-8">
      ...
      <div className="bg-gray-900/50 p-6 rounded-xl border border-gray-800 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1">Project ID (URL slug)</label>
          ...
          <div>
            ...
            <input type="text" name="year" value={formData.year} onChange={handleChange} className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white" />
          </div>
        </div>
      ...
      <div className="bg-gray-900/50 p-6 rounded-xl border border-gray-800 space-y-2">
        ...
        <DynamicStringArrayInput
          ...
          items={formData.technical_details}
          onChange={(newItems) => setFormData({ ...formData, technical_details: newItems })}
        />
        <DynamicStringArrayInput
          label="Challenges"
          items={formData.challenges}
          onChange={(newItems) => setFormData({ ...formData, challenges: newItems })}
        />
        <DynamicStringArrayInput
          label="Outcomes"
          items={formData.outcomes}
          onChange={(newItems) => setFormData({ ...formData, outcomes: newItems })}
        />
        <DynamicStringArrayInput
          label="Personal Contribution"
          items={formData.personal_contribution}
          onChange={(newItems) => setFormData({ ...formData, personal_contribution: newItems })}
        />
      </div>
      ...
      <div className="bg-gray-900/50 p-6 rounded-xl border border-gray-800 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-400 mb-1">Design Process (Markdown supported)</label>
          <textarea name="design_process" value={formData.design_process} onChange={handleChange} rows={5} className="w-full bg-gray-800 border border-gray-700 rounded p-2 text-white font-mono text-sm" />
        </div>
      </div>
